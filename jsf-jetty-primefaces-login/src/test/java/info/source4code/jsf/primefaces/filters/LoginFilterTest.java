package info.source4code.jsf.primefaces.filters;

import static org.junit.Assert.fail;
import static org.mockito.Mockito.verify;
import static org.mockito.Mockito.when;
import info.source4code.jsf.primefaces.view.UserManager;
...
public class LoginFilterTest {
    ...
    @Test
    public void testDoFilterUserManagerNull() throws IOException,
            ServletException {
        userManager = null;
        when(httpSession.getAttribute("userManager")).thenReturn(userManager);

        loginFilter.doFilter(httpServletRequest, httpServletResponse,
                filterChain);

        verify(httpServletResponse).sendRedirect(
                "http://localhost:9090/s4c/login.xhtml");
    }
